...
@RestController
@RequestMapping(path = "/dynamic-flags")
public class DynamicFlagRestController {
  ...
  @Autowired
  public DynamicFlagRestController(
      TrainingDefinitionService trainingDefinitionService, DynamicFlagService dynamicFlagService) {
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            code = 200,
            message = "The configuration has been retrieved.",
            response = DynamicFlagConfigDTO.class),
        @ApiResponse(
            code = 404,
  ...
  public ResponseEntity<DynamicFlagConfigDTO> getConfig(
    ...
    DynamicFlagConfigDTO config =
        new DynamicFlagConfigDTO(
            td.isEnableDynamicFlag(), td.getFlagChangeInterval(), td.getInitialSecret());
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            code = 400,
            message = "The provided configuration is not valid.",
            response = ApiError.class),
        @ApiResponse(
            code = 404,
            message = "The training definition has not been found.",
  ...
  public ResponseEntity<Void> updateConfig(
      @ApiParam(value = "ID of training definition", required = true) @PathVariable("definitionId")
          Long id,
      @ApiParam(value = "Dynamic flag configuration") @RequestBody @Valid
          DynamicFlagConfigDTO config) {
    TrainingDefinition td = trainingDefinitionService.findById(id);
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            code = 200,
            message = "The current flag has been retrieved.",
            response = String.class),
        @ApiResponse(
            code = 400,
            message = "Dynamic flag is not enabled.",
            response = ApiError.class),
        @ApiResponse(
            code = 404,
            message = "The training definition has not been found.",
  ...
  @PreAuthorize(
      "hasAuthority(T(cz.cyberrange.platform.training.service.enums.RoleTypeSecurity).ROLE_TRAINING_ADMINISTRATOR)"
          + "or @securityService.isDesignerOfGivenTrainingDefinition(#id)")
  public ResponseEntity<String> getCurrentFlag(
      @ApiParam(value = "ID of training definition", required = true) @PathVariable("definitionId")
          Long id) {
    TrainingDefinition td = trainingDefinitionService.findById(id);
    ...
    String flag =
        dynamicFlagService.generateFlag(td.getInitialSecret(), td.getFlagChangeInterval());
