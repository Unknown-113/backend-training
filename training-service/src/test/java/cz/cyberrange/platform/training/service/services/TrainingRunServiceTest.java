...
public class TrainingRunServiceTest {
  ...
  @Test
  public void isCorrectAnswerWithDynamicFlag() {
    // Setup training definition with dynamic flag enabled
    TrainingDefinition dynamicDefinition = testDataFactory.getReleasedDefinition();
    dynamicDefinition.setId(3L);
    dynamicDefinition.setEnableDynamicFlag(true);
    dynamicDefinition.setFlagChangeInterval(5);
    dynamicDefinition.setInitialSecret("testSecret");

    TrainingInstance dynamicInstance = testDataFactory.getOngoingInstance();
    dynamicInstance.setId(3L);
    dynamicInstance.setTrainingDefinition(dynamicDefinition);

    TrainingRun dynamicRun = new TrainingRun();
    dynamicRun.setId(3L);
    dynamicRun.setTrainingInstance(dynamicInstance);
    dynamicRun.setCurrentLevel(trainingLevel);
    dynamicRun.setMaxLevelScore(10);
    dynamicRun.setCurrentPenalty(0);

    given(trainingRunRepository.findByIdWithLevel(dynamicRun.getId()))
        .willReturn(Optional.of(dynamicRun));
    given(dynamicFlagService.generateFlag("testSecret", 5)).willReturn("dynamicFlag123");

    boolean isCorrect = trainingRunService.isCorrectAnswer(dynamicRun.getId(), "dynamicFlag123");
    assertTrue(isCorrect);
    assertTrue(dynamicRun.isLevelAnswered());
    verify(dynamicFlagService).generateFlag("testSecret", 5);
  }
  ...
  public void testCheckRunEventLogging() {
    ...
    given(elasticsearchApiService.findAllEventsFromTrainingRun(trainingRun2))
  ...
  public void testCheckRunCommandLogging() {
    ...
    given(elasticsearchApiService.findAllConsoleCommandsBySandbox(anyString()))
        .willReturn(List.of());
    given(
            elasticsearchApiService.findAllConsoleCommandsByAccessTokenAndUserId(
                anyString(), anyLong()))
