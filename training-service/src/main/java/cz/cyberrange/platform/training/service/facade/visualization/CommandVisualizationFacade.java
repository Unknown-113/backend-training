...
public class CommandVisualizationFacade {
  ...
  private List<CommandDTO> elasticCommandsToCommandDTOlist(
    ...
    elasticCommands.stream()
        .map(
            elasticCommand -> objectMapper.convertValue(elasticCommand, ElasticSearchCommand.class))
        .forEach(
            command -> commandDTOS.add(elasticSearchCommandToCommandDTO(command, runStartTime)));
    return commandDTOS;
  }

  private CommandDTO elasticSearchCommandToCommandDTO(
      ElasticSearchCommand elasticSearchCommand, LocalDateTime runStartTime) {
